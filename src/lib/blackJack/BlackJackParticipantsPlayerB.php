 ...
 */
class BlackJackParticipantsPlayerB implements ParticipantsRule
{
    /**
     * 初回の手札を取得
     *
    ...
    public function getInitHand(array $gameDeck): array
    {
        ...
        $initHands = [];
        for ($i = 0; $i <= 1; $i++) {
            echo 'プレイヤーBの引いたカードは' . BlackJackDeck::CARD_KIND_NAME[substr($gameDeck[$i], 0, 1)] . 'の' .
    ...
    public function getHand(array $gameDeck, array $initHands): array
    {
        ...
        while ($totalRank <= 17) {
            ...
            echo 'プレイヤーBの引いたカードは' . BlackJackDeck::CARD_KIND_NAME[substr($gameDeck[$count], 0, 1)] . 'の' .
