 ...
 */
class BlackJackJudgmentOneOnThree implements JudgmentRule
{
    /* @var array ゲーム参加者 */
    ...
    public function getJudgment(array $judgment): void
    {
        ...
        for ($i = 0; $i <= 2; $i++) {
            ...
            } elseif ($this->isPlayerWin($judgment[self::GAME_PARTICIPANTS[$i]]['status'], $judgment[self::GAME_PARTICIPANTS[3]]['status'])) {
                echo self::GAME_PARTICIPANTS[$i] . 'の勝ちです！' . PHP_EOL;
            } elseif ($this->isPlayerWin($judgment[self::GAME_PARTICIPANTS[$i]]['totalRank'], $judgment[self::GAME_PARTICIPANTS[3]]['totalRank'])) {
                echo self::GAME_PARTICIPANTS[$i] . 'の勝ちです！' . PHP_EOL;
            } elseif ($this->isDealerWin($judgment[self::GAME_PARTICIPANTS[$i]]['totalRank'], $judgment[self::GAME_PARTICIPANTS[3]]['totalRank'])) {
                echo self::GAME_PARTICIPANTS[$i] . 'の負けです。' . PHP_EOL;
            } elseif ($this->isDraw($judgment[self::GAME_PARTICIPANTS[$i]]['totalRank'], $judgment[self::GAME_PARTICIPANTS[3]]['totalRank'])) {
                echo '引き分けです。' . PHP_EOL;
            }
        }
    }
