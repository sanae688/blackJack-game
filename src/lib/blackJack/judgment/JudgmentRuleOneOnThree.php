 ...
 */
class JudgmentRuleOneOnThree implements IJudgmentRule
{
    /* @var array ゲーム参加者 */
    ...
    public function showJudgment(array $judgment): void
    {
        ...
        foreach (self::PLAYER as $player) {
            ...
            } elseif ($this->isPlayerWin($judgment[$player]['status'], $judgment[self::GAME_PARTICIPANTS[3]]['status'])) {
                echo $player . 'の勝ちです！' . PHP_EOL;
            } elseif ($this->isPlayerWin($judgment[$player]['totalRank'], $judgment[self::GAME_PARTICIPANTS[3]]['totalRank'])) {
                echo $player . 'の勝ちです！' . PHP_EOL;
            } elseif ($this->isDealerWin($judgment[$player]['totalRank'], $judgment[self::GAME_PARTICIPANTS[3]]['totalRank'])) {
                echo $player . 'の負けです。' . PHP_EOL;
            } elseif ($this->isDraw($judgment[$player]['totalRank'], $judgment[self::GAME_PARTICIPANTS[3]]['totalRank'])) {
